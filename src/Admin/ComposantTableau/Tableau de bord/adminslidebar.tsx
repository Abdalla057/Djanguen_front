      ...
      >
        ...
        <AnimatePresence>
          {isActive && (
            <motion.div
              layoutId="activeNav"
              className="absolute inset-0 bg-[#6D9773] text-white"
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
              exit={{ opacity: 0 }}
            />
        ...
        <div className="relative z-10 flex items-center gap-2.5 w-full">
          ...
          {/* Badge — cta */}
          {item.badge && (
            <span className="px-1.5 py-0.5 text-[10px] font-bold bg-white text-[#1a1a2e] rounded-full">
              {item.badge}
            </span>
          )}
  ...
  return (
    <>
      ...
      <motion.aside
        ...
        className={`fixed top-0 left-2 h-screen w-60 flex flex-col z-50 overflow-hidden  ${
      ...
      >
        ...
        <div className="absolute inset-2 rounded-2xl z-[1] bg-white/60 backdrop-blur-sm border " />
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div className="relative group">
                <div className="absolute -inset-1 rounded-xl blur opacity-40 group-hover:opacity-70 transition duration-500 " />
                <div className="relative w-9 h-9 rounded-xl flex items-center justify-center bg-[#ffba00] border border-[#ffba00] shadow-sm">
                  <BookOpen className="w-10 h-10 text-white p-2" />
                </div>
              </div>

              {/* Nom — textDark + cta accent */}
              <div className="border border-white rounded-full shadow-2xl bg-white h-8 w-full">
                <h1 className="text-2xl font-bold tracking-tight leading-none text-[#ffBA00]">
                  Dian<span className="text-[#0c3b2e]">gueen</span>
                </h1>
              </div>
            </div>
        ...
        <div className="relative z-10 p-3 mt-auto">
          ...
          <button
            ...
            className="w-full flex items-center justify-center gap-2 py-2.5 rounded-xl font-bold text-xs transition-all duration-300 bg-white border border-[#0c3b2e] text-[#0c3b2e] hover:bg-[#6D9773] hover:text-white"
          >
    ...
    >
      ...
      <div className="flex gap-2">
        ...
        <button
          ...
          className="flex-1 py-2.5 rounded-xl text-xs font-semibold bg-gradient-to-r from-white to-red-200 text-red-500 hover:opacity-90 transition-all"
        >
          Confirmer
        </button>
      </div>

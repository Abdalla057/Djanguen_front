...
const CarteLivre: React.FC<Props> = ({ livre, modeAffichage,  onNaviguer }) => {
  ...
  return (
    <div onClick={() => onNaviguer(livre.id)} className="group cursor-pointer">
      ...
      {/* Couverture */}
      <div
        className="w-full rounded-xl overflow-hidden shadow-2xl group-hover:shadow-white transition-all duration-300 group-hover:-translate-y-1"
      ...
      {!estCompacte && (
        <div className="mt-2 px-0.5">
          <p className="text-black font-medium leading-snug line-clamp-2"
            style={{ fontSize: 13 }}>
            {livre.titre ?? "Titre inconnu"}
          </p>
          {livre.auteur && (
            <p className="text-black mt-0.5 truncate" style={{ fontSize: 11 }}>
              {livre.auteur}
            </p>
          )}
          ...
          {sauvegarde && (
            <p className="text-[10px] text-red-200 mt-0.5">Sauvegarde...</p>
          )}
        </div>
      )}
    </div>

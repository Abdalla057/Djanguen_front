...
const IndexBibliotheque: React.FC<Props> = ({ role = "utilisateur" }) => {
  ...
  const {
    ...
    categorieActive, basculerCategorie, reinitialiserCategorie,
    livresParCategorie, categories, totalLivres,
  } = useFiltrageLivres(livres);

  // ← La route change selon le rôle
  const naviguerVersLivre = useCallback(
    (id: number) => naviguer(`/${role}/livre/${id}/pages`),
    [naviguer, role]
  );
  ...
  if (erreur) return (
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-b from-[#fdf6f0] to-[#fde8d8]">
      <div className="bg-white/80 border border-red-200 rounded-2xl p-8 max-w-sm text-center">
        ...
        <button onClick={() => window.location.reload()} className="mt-4 px-5 py-2 rounded-xl bg-[#e8734a] text-white text-sm font-medium">
          Réessayer
        </button>
      </div>
    </div>
          ...
          <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4 mb-6">
            <div>
              <h1 className="text-3xl sm:text-4xl font-bold text-[#3b1f0e] tracking-tight">
                {role === "admin" ? "Gestion Bibliothèque" : "Ma Bibliothèque"}
              </h1>
            ...
            <div className="flex items-center gap-2 bg-white/60 rounded-xl p-1 border border-[#0c3b2e]">
              {(["grille", "compacte"] as ModeAffichage[]).map(mode => (
                <button
                  key={mode}
                  onClick={() => setModeAffichage(mode)}
                  className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-all ${
                    modeAffichage === mode ? "bg-[#ffBA00] text-white shadow-sm" : "text-black hover:bg-white"
                  }`}
                >
            ...
            <div className="flex flex-wrap gap-2">
              <button
                ...
                className={`px-4 py-1.5 rounded-full text-xs font-medium border transition-all ${
                  !categorieActive ? "bg-[#ffBA00] text-white border-[#ffBA00]" : "bg-white/60 text-black border-[#f0cbb0] hover:border-[#e8734a]"
                }`}
              >
                Tous
              </button>
              {categories.map(cat => (
                <button
                  ...
                  className={`px-4 py-1.5 rounded-full text-xs font-medium border transition-all ${
                    categorieActive === cat ? "bg-[#ffBA00] text-white border-[#ffBA00]" : "bg-white text-black border-[#f0cbb0] hover:border-[#e8734a]"
                  }`}
                >
                  {cat}
                </button>
...
export default IndexBibliotheque;